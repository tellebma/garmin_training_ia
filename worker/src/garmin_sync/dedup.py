...
The query is scoped to `source='garmin'` on purpose: it must never match a
previously-inserted Strava row (e.g. the one this same activity created on
first sync). Matching a Strava row would make `aspect_type='update'` webhook
events look like duplicates of themselves, silently dropping every Strava
activity edit.
"""
